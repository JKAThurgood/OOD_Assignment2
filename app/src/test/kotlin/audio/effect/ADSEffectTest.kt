...
class ADSEffectTest {
    ...
    @Test
    fun `decay phase uses decay interpolation`() {
        val effect = ADSEffect(
            attackEnd = 1.0,
            decayEnd = 2.0,
            sustain = 0.5,
        ...
        // 1.0 -> 0.5 over 2 seconds, halfway = 0.75
        assertEquals(0.75, sample, 1e-10)
    }

    @Test
    fun `decay phase with zero decay end uses sustain`() {
        val effect = ADSEffect(
            attackEnd = 1.0,
            decayEnd = 0.0,
            sustain = 0.5,
